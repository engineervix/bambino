...
export function useErrorHandling() {
  ...
  async function withErrorHandling(asyncOperation, options = {}) {
    const { loadingState = true, clearPreviousError = true, onSuccess, onError } = options;
...
export function useFormHandling() {
  ...
  async function handleFormSubmit(formRef, submitFunction, options = {}) {
    const { validateForm = true, successMessage = "Saved successfully!", onSuccess, onError } = options;
    ...
    return await errorHandling.withErrorHandling(submitFunction, {
      onSuccess: (result) => {
        if (onSuccess) {
