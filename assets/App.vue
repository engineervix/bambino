<template>
  <v-app>
    ...
    <v-app-bar v-if="showTopBar" app color="surface" elevation="1" density="comfortable">
      <v-toolbar-title class="text-h6 font-weight-medium d-flex align-center">
        <v-img src="/baby.svg" alt="Logo" width="32" height="32" cover class="d-none d-md-flex" />
      </v-toolbar-title>
      ...
      <div class="d-flex align-center">
        ...
        <v-menu transition="fade-transition" location="bottom end">
          <template #activator="{ props }">
            <v-btn v-bind="props" variant="text" :class="{ 'text-primary': activeTab === 'account' }" size="small">
    ...
    <v-bottom-navigation v-if="showBottomNav" v-model="activeTab" grow bg-color="surface">
      <v-btn v-for="item in navItems" :key="'bottom-' + item.value" :value="item.value" :to="item.to">
